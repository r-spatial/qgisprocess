...
`qgis_pipe()` circumvents this by accepting a `.data` object as its first argument, and pipes this data object into the first argument of a geoalgorithm assuming that the specified geoalgorithm in fact needs a data input object as its first argument.
...
In case of a `qgis_result` object, `qgis_pipe()` automatically tries to select an element named `OUTPUT`.
...
To turn of this behavior, set `.clean` to `FALSE`.

Of course, you can also use the native pipe.
