...
s = c(lapply(out[c("AREA", "SLOPE")], \(x) terra::rast(x[1])))
...
args = rlang::set_names(paste0(tempdir(), "/", names(s), ".sdat"), rep("GRIDS", length(s)))
data(random_points, package = "spDataLarge")
out = qgis_run_algorithm("saga:addrastervaluestopoints",
                         SHAPES = random_points,
                         GRIDS = as.raster(s$cslope),
                         GRIDS = as.raster(s$ndvi),
                         GRIDS = as.raster(s$log_carea),
                         RESAMPLING = 0,
