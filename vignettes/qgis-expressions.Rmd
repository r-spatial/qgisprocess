...
- An algorithm argument expects a QGIS expression string.
...
An example is provided by the `DISTANCE` argument of `native:buffer`.
...
# Examples where an argument is a QGIS expression string
...
Referring the lake border is a bit more involved, since it needs several QGIS functions.
...
Note: the `load_layer()` function is only available since QGIS 3.30!
...
The `load_layer()` approach since QGIS 3.30 avoids the need for a QGIS project.
