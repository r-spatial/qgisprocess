...
res <- qgis_run_algorithm("native:buffer", INPUT = random_points, DISTANCE = 50)
```
...
The `res` object is of class `qgis_result` and contains the path to the output file created by `qgis_process` (when not explicitly setting an output object, **qgisprocess** creates it automatically for you).
The output filepath be extracted with `qgis_output()`.
It also contains debugging information about used algorithm, input arguments and messages from the processing step; see `?qgis_result_status` to extract this easily.

For QGIS 3.24 and later, `qgis_run_algorithm()` passes the input arguments to QGIS as a JSON string.
The JSON input string is also included in the `res` object.
...
```{r inspect}
# inspect the res object
class(res)
names(res)
res # only prints the output element(s)
```
...
buf <- read_sf(qgis_output(res, "OUTPUT"))
...
# run the function
res <- qgis_buffer(INPUT = random_points, DISTANCE = 50)
```
...
qgis_arguments("sagang:addrastervaluestopoints")
```
...
`qgis_pipe()` circumvents this by accepting a `.data` object as its first argument, and pipes this data object into the the first argument of a geoalgorithm assuming that the specified geoalgorithm needs a data input object as its first argument.

```{r buf-pipe}
system.file("longlake/longlake_depth.gpkg", package = "qgisprocess") |>
  qgis_pipe("native:buffer", DISTANCE = 50)
```

If `.data` is a `qgis_result` object, `qgis_pipe()` automatically tries to select an element named `OUTPUT`.
...
# in case you need to enable the SAGA next generation algorithms, run the following lines:
...
qgis_run_algorithm(algorithm = "sagang:sinkremoval", DEM = dem,
                   METHOD = 1) |>
  qgis_pipe("sagang:sagawetnessindex", .select = "DEM_PREPROC")
```

When piping, `qgis_pipe()` automatically cleans up after you by deleting intermediate results.
...
This avoids ambiguity and allows for greater flexibility though it might not be as convenient as `qgis_pipe()`.
For example, intermediate results remain on disk for the duration of your R session.

```{r native pipe}
res <- qgis_run_algorithm(algorithm = "sagang:sinkremoval", DEM = dem, 
                         METHOD = 1) |>
  _$DEM_PREPROC[1] |>
