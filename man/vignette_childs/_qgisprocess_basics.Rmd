...
The `result` object is of class `qgis_result` and contains the path to the output file created by `qgis_process` (when not explicitly setting an output object, **qgisprocess** creates it automatically for you).
The output filepath be extracted with `qgis_output()`.
The `result` object also contains debugging information about used algorithm, input arguments and messages from the processing step; see `?qgis_result_status` to extract this easily.

For QGIS 3.24 and later, `qgis_run_algorithm()` passes the input arguments to QGIS as a JSON string.
The JSON input string is also included in the `result` object.
...
buf <- read_sf(qgis_output(result, "OUTPUT"))
